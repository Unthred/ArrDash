...
5. **Preview** then **Sync now**. Each sync processes a batch of mark-watched / library-push writes; re-run until counts settle. Background auto-sync runs once daily at **04:00 Europe/London** (not hourly).
