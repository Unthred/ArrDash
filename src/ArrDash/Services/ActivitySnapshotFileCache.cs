...
public sealed class ActivitySnapshotFileCache(IOptions<DatabaseOptions> dbOptions, ILogger<ActivitySnapshotFileCache> logger)
{
    ...
    private readonly string _root = Path.Combine(ResolveConfigRoot(dbOptions.Value.SqlitePath), "cache", "activity");
    ...
    public async Task<CachedActivitySnapshot?> TryGetAsync(string key, CancellationToken ct = default)
    {
        ...
        try
        {
            ...
            var envelope = await JsonSerializer.DeserializeAsync<CacheEnvelope<ActivityAnalyticsSnapshot>>(stream, JsonOptions, ct);
            if (envelope?.Snapshot is null)
                return null;

            return new CachedActivitySnapshot(envelope.Snapshot, envelope.UpdatedAtUtc);
        }
        catch (Exception ex) {
    ...
    public async Task SaveAsync(string key, ActivityAnalyticsSnapshot snapshot, CancellationToken ct = default)
    {
        Directory.CreateDirectory(_root);
        var path = PathFor(key);
        var temp = path + ".tmp";
        var envelope = new CacheEnvelope<ActivityAnalyticsSnapshot>(snapshot, DateTimeOffset.UtcNow);
    ...
    public sealed record CachedActivitySnapshot(ActivityAnalyticsSnapshot Snapshot, DateTimeOffset UpdatedAtUtc)
    {
